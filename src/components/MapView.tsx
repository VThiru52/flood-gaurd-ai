...
import { kadapaFloodZones } from "@/data/kadapaFloodData";
...
function MapView() {
  const criticalCount = kadapaFloodZones.filter(z => z.risk === "critical").length;
    ...
    <div className="glass-panel relative overflow-hidden h-[500px] animate-fade-in">
      ...
      <div className="h-full flex flex-col pt-12 px-4 pb-4">
          ...
          {kadapaFloodZones.map((zone) => (
            <div key={zone.name} className="flex items-center gap-3 p-3 rounded-lg bg-secondary/30 hover:bg-secondary/50 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <span className="text-xs text-foreground block truncate">{zone.name}</span>
                <span className="text-[10px] text-muted-foreground">Zone: {zone.zone} · {zone.lat.toFixed(4)}°N, {zone.lng.toFixed(4)}°E</span>
              </div>
        ...
        <div className="flex items-center gap-4 mt-3 pt-3 border-t border-border/30">
          ...
          <span className="text-[10px] text-muted-foreground ml-auto">Source: Kadapa Master Plan 2041 + IDF Analysis</span>
        </div>
      </div>
    </div>
  );
